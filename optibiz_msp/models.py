...
class optibiz_saleorder(osv.osv):
    ...
    def _amount_all(self, field_name, args):
        ...
        uid = logged_in.id
        option = -1;
        sp = self.pool.get('res.users').has_group(cr,uid,'optibiz_msp.group_sell_on_selling_price')
        msp = self.pool.get('res.users').has_group(cr,uid,'optibiz_msp.group_sell_on_minimum_selling_price')
        if msp:
            option = 1
        elif sp:
            option = 0
